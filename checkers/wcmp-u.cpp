...
{
    setName("compare unordered sequences of tokens");
    ...
    while (!ans.seekEof())
        ja.push_back(ans.readWord());

    while (!ouf.seekEof())
        pa.push_back(ouf.readWord());
    ...
    if (ja.size() != 1)
        if (ja.empty())
            ...
            message = vtos(ja.size()) + " tokens (in increasing order):";
    else
        message = vtos(ja.size()) + " token:";
